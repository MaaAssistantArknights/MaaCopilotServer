...
namespace MaaCopilotServer.Application.Test.Common.Behaviours
{
    ...
    public class ValidationBehaviourTest
    {
        ...
        /// <summary>
        /// Tests <see cref="ValidationBehaviour{TRequest, TResponse}.Handle(TRequest, CancellationToken, RequestHandlerDelegate{TResponse})"/> with empty valiators.
        /// </summary>
        /// <returns></returns>
        [TestMethod]
        public async Task TestHandle_EmptyValidators()
        {
            this._validators = new List<IValidator<IRequest<string>>>();
            var behaviour = new ValidationBehaviour<IRequest<string>, string>(this._validators, this._currentUserService);
        ...
        /// <summary>
        /// Tests <see cref="ValidationBehaviour{TRequest, TResponse}.Handle(TRequest, CancellationToken, RequestHandlerDelegate{TResponse})"/> with validators.
        /// </summary>
        ...
        public async Task TestHandle_Validators(bool validator1Passed, bool validator2Passed, bool expectException)
        {
            ...
            this._validators = new List<IValidator<IRequest<string>>> { validator1, validator2 };
            var behaviour = new ValidationBehaviour<IRequest<string>, string>(this._validators, this._currentUserService);
            var action = async () => await behaviour.Handle(Substitute.For<IRequest<string>>(),
                                                            new CancellationToken(),
                                                            () => Task.FromResult(string.Empty));
    ...
    /// The mock validator.
    /// </summary>
    class TestValidator : IValidator<IRequest<string>>
    {
        /// <summary>
        ...
        public Task<ValidationResult> ValidateAsync(IValidationContext context, CancellationToken cancellation = default)
        {
            ...
            var resultFailed = new ValidationResult(new List<ValidationFailure>
            {
                new ValidationFailure()
            });
            return Task.FromResult(this._passed ? resultPassed : resultFailed);
        }
    }
}
